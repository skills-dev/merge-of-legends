...
- Contains a one-shot `update_readme` job that runs **only** on `push` events and **only** when `github.repository != 'skills-dev/merge-of-legends'`. On the first push after the template is copied to a new handle, it renders `.github/markdown-templates/readme/start-game.md` (vars: `game-title`, `issues_url` = repo issues tab URL, `login` = repository owner) through the shared `./.github/actions/render-markdown-template` action and publishes `README.md` via the GitHub Contents API. The step is idempotent — it no-ops when the rendered content matches the existing README — and does **not** run on `workflow_call`, `issues`, `issue_comment`, or `workflow_dispatch`, so normal quest flow never rewrites the README.
